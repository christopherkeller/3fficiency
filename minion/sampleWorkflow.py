...
def askStatus(username):
	c = connectDB()
	print """Hi %s,\n""" % username
	c.execute("""SELECT predicted_status FROM status WHERE user_id=(SELECT id FROM user WHERE user_name='%s')
		AND date > DATE_SUB(CURDATE(), INTERVAL 1 WEEK)""" % args[0])
	result=c.fetchall()
	for record in result:
		print """Last week you said you'd like to "%s". Did this get done?\n""" % record[0].rstrip()
		lastWeek = raw_input()
